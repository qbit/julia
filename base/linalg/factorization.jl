...
function eigfact!{T<:BlasReal}(A::StridedMatrix{T}; balance::Symbol=:balance)
    ...
    A, WR, WI, VL, VR, tmp1, tmp2, tmp3, tmp4, tmp5 = LAPACK.geevx!(balance == :balance ? 'B' : (balance == :diagonal ? 'S' : (balance == :permute ? 'P' : (balance == :nobalance ? 'N' : throw(ArgumentError("balance must be either ':balance', 'diagonal', 'permute' or 'nobalance'"))))), 'N', 'V', 'N', A)
